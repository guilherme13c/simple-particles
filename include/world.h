...
class World {
  ...
  public:
    World(float max_x, float min_x, float max_y, float min_y, float max_z,
        ...
        if (save_states) {
            ...
            save_configuration();
            stop_worker = false;
            worker_thread = std::thread(&World::save_state_to_file, this);
        }
    }

    World(uint64_t N, float max_x, float min_x, float max_y, float min_y,
